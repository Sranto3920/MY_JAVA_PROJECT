...
public class MedicineResultsPage {
    ...
    public BorderPane getView() {
        ...
        TableColumn<SelectionRow, String> shopCol = new TableColumn<>("Shop");
        shopCol.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getMedicine().getShop()));

        TableColumn<SelectionRow, String> locCol = new TableColumn<>("Location");
        locCol.setCellValueFactory(cd -> new javafx.beans.property.SimpleStringProperty(cd.getValue().getMedicine().getLocation()));

        TableColumn<SelectionRow, String> brandCol = new TableColumn<>("Brand");
        ...
        table.getColumns().addAll(selCol, shopCol, locCol, brandCol, priceCol, qtyCol);
    ...
    private void showFilterDialog() {
        ...
        Set<String> locs = repo.distinctLocations();
        Set<String> shops = repo.distinctShops();
        Set<String> brands = repo.distinctBrands();

        ComboBox<String> locBox = new ComboBox<>(); locBox.getItems().add("All"); locBox.getItems().addAll(locs);
        ComboBox<String> shopBox = new ComboBox<>(); shopBox.getItems().add("All"); shopBox.getItems().addAll(shops);
        ComboBox<String> brandBox = new ComboBox<>(); brandBox.getItems().add("All"); brandBox.getItems().addAll(brands);
        locBox.getSelectionModel().selectFirst();
        ...
        gp.addRow(0, new Label("Location:"), locBox);
        gp.addRow(1, new Label("Shop:"), shopBox);
        gp.addRow(2, new Label("Brand:"), brandBox);
        dlg.getDialogPane().setContent(gp);

        dlg.setResultConverter(bt -> {
            if (bt == apply) return new FilterResult(val(locBox), val(shopBox), val(brandBox));
